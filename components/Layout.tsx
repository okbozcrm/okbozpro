...
const Layout: React.FC<LayoutProps> = ({ children, role, onLogout }) => {
  ...
  useEffect(() => {
    ...
    // Check for login flag to show welcome popup
    ...
    if (role === UserRole.ADMIN) {
      ...
      try {
       ...
       try {
         ...
         // Try Corporate Staff if not found
         if (!emp) {
            const accounts: CorporateAccount[] = JSON.parse(localStorage.getItem('corporate_accounts') || '[]');
         ...
         if (emp) {
             foundName = emp.name;
             foundRole = emp.role;
             if (emp.moduleAccess && Array.isArray(emp.moduleAccess)) {
  ...
  const userLinks = useMemo(() => {
    ...
    // Restricted Links (Permission Based)
    ...
    employeePermissions.forEach(perm => {
        if (restrictedLinksMap[perm]) {
            addedLinks.push(restrictedLinksMap[perm]);
        }
    });
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-500">
              <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 flex flex-col items-center justify-center text-center max-w-sm w-full animate-in zoom-in-95 duration-500 border border-gray-200 dark:border-gray-700">
                  <div className="mb-4 text-emerald-500 animate-bounce">
                      {greeting.includes('Morning') ? <Sunrise className="w-16 h-16" /> : greeting.includes('Afternoon') ? <SunIcon className="w-16 h-16" /> : <MoonIcon className="w-16 h-16" />}
                  </div>
                  ...
                  <button 
                    ...
                    className="mt-2 text-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors"
                  >
                    Dismiss
                  </button>
              </div>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-white dark:bg-gray-850 border-r border-gray-100 dark:border-gray-750 transition-transform duration-200 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}
      >
        {/* Sidebar Header */}
          ...
          {(role === UserRole.ADMIN || role === UserRole.CORPORATE) && (
             <div className="mt-4 p-2 border-t border-gray-100 dark:border-gray-700">
                <button 
                   ...
                   className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-750 transition-colors"
                >
                   <Edit2 className="w-4 h-4 text-gray-500 dark:text-gray-400" /> 
                   {isEditingSidebar ? 'Done Editing' : 'Edit Sidebar'}
                </button>
             </div>
          )}
          {/* Install App Button in Sidebar for Employee */}
          {role === UserRole.EMPLOYEE && isInstallable && (
             <div className="mt-4 p-2 border-t border-gray-100 dark:border-gray-700">
                <button 
                   onClick={handleInstallClick}
                   className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors"
                >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={notificationRef}>
                <button
                    ...
                    className={`relative p-2 rounded-full text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ${unreadCount > 0 && !notificationsOpen ? 'animate-blink-bell' : ''}`}
                    title="Notifications"
                >
                    <BellDot className="w-5 h-5" />
                    {unreadCount > 0 && (
                        <span className="absolute top-0 right-0 block h-2 w-2 rounded-full bg-red-500 ring-2 ring-white dark:ring-gray-850"></span>
                    )}
                </button>
                {notificationsOpen && (
                    <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-750 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                        <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                            ...
                            <button onClick={markAllNotificationsAsRead} className="text-xs text-blue-600 dark:text-blue-400 hover:underline">
                                Mark All as Read
                            </button>
                        </div>
                            ...
                            {notifications.length === 0 ? (
                                ...
                                notifications.map(notif => (
                                    <div
                                        ...
                                        className={`p-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer ${notif.read ? 'opacity-70' : 'font-medium bg-blue-50 dark:bg-blue-950'}`}
                                    >
            ...
            <button 
              onClick={onLogout}
              className="p-2 rounded-full text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors ml-1"
              title="Logout"
            >
