...
const convertToCSV = (objArray: any[]) => {
  ...
  array.forEach((item: any) => Object.keys(item).forEach(key => allKeys.add(key)));
       ...
       <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden relative">
          ...
          <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-6">
             <div className="flex items-start gap-4">
                 ...
                 <div>
                     <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">Google Drive Sync<span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full border border-green-200">Auto-Format</span></h3>
                     <p className="text-gray-500 mt-1 max-w-lg">Automatically compile all modules (Reports, Payments, Staff, etc.) into a single master Excel-compatible file and prepare it for your Google Drive.</p>
                 </div>
             </div>
             <button onClick={handleGoogleDriveSync} disabled={driveSyncing} className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg flex items-center gap-3 transition-all transform hover:scale-105 disabled:opacity-70 disabled:cursor-not-allowed">
                 {driveSyncing ? <><Loader2 className="w-6 h-6 animate-spin" /> Preparing Cloud Sync...</> : <><Cloud className="w-6 h-6" /> Sync to Drive</>}
             </button>
          </div>
       </div>
       <div className="flex justify-end gap-3">
            <button onClick={() => window.location.reload()} className="px-4 py-2 text-sm text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center gap-2"><RefreshCw className="w-4 h-4" /> Refresh Data</button>
            <button onClick={handleDownloadAll} disabled={isLoading} className="px-6 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-sm flex items-center gap-2">{isLoading ? <Loader2 className="w-4 h-4 animate-spin"/> : <Download className="w-4 h-4" />} Download All (ZIP/Multiple)</button>
       </div>
           ...
           {MODULES.map((mod) => (
               <div key={mod.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow group">
                   <div className="flex justify-between items-start mb-4">
                       <div className="p-3 bg-gray-50 rounded-lg group-hover:bg-emerald-50 transition-colors"><FileSpreadsheet className="w-6 h-6 text-gray-500 group-hover:text-emerald-600" /></div>
                       <span className={`text-xs font-bold px-2 py-1 rounded-full ${dataStats[mod.id] > 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{dataStats[mod.id]} Records</span>
                   </div>
                   <h4 className="font-bold text-gray-800 text-lg mb-1">{mod.label}</h4>
                   <p className="text-xs text-gray-400 mb-6">Key: {mod.key}</p>
                   <button onClick={() => handleDownload(mod.id)} disabled={dataStats[mod.id] === 0} className="w-full py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-emerald-600 hover:border-emerald-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><Download className="w-4 h-4" /> Download CSV</button>
               </div>
           ))}
       </div>
    </div>
