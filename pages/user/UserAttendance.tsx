...
const UserAttendance: React.FC<UserAttendanceProps> = ({ isAdmin = false }) => {
  ...
  const saveStateToHistory = (newData: DailyAttendance[]) => {
  ...
  const handleSaveChanges = () => {
      ...
      // Update history if current employee
      if (targetEmpId === selectedEmployee?.id) {
          saveStateToHistory(updatedMonthData);
      }
  ...
  const handleMarkStatusRange = async (status: AttendanceStatus) => {
    ...
    saveStateToHistory(attendanceData);
            ...
            {isAdmin && selectedEmployee && (
                <div className="flex flex-wrap gap-2">
                    <button 
                        ...
                        className={`p-2 rounded-xl border transition-all ${historyIndex > 0 ? 'bg-white hover:bg-gray-50 text-gray-600' : 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed'}`}
                        title="Undo"
                    >
                    ...
                    <button 
                        ...
                        className={`p-2 rounded-xl border transition-all ${historyIndex < history.length - 1 ? 'bg-white hover:bg-gray-50 text-gray-600' : 'bg-gray-50 text-gray-300 border-gray-100 cursor-not-allowed'}`}
                        title="Redo"
                    >
